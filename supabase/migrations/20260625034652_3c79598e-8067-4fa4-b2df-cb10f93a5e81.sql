...
-- Tighten the insert policy so the new optional columns are length-checked too.
DROP POLICY IF EXISTS "Anyone can submit a valid booking enquiry" ON public.booking_enquiries;
...
WITH CHECK (
  ...
  AND (attachment_urls IS NULL OR array_length(attachment_urls, 1) <= 10)
);
